...
def main():
    ...
    env = RecordRewWrapper(env, rew_log_dir=f"./ckp/log/rew/{params['env_name']}/ppo_fvrl_decay_2.0_50w", avg_n=10)
    ...
    scorer_func = make_linear_decayable_scorer_func(0.2, 500_000)
    ...
    model = FVRL_PPO_2(
        ...
        verbose=1, tensorboard_log=f"./ckp/log/{params['env_name']}_ppo_fvrl_decay_2.0_50w/", gae_lambda=params["gae_lambda"],
    )
    print(model.policy)
    model.learn(total_timesteps=1_000_000)
    model.save(f"./ckp/pth/{params['env_name']}_ppo_fvrl_decay_2.0_50w_100w.pth")
    env.close()
